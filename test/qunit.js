...
test("on", 3, function(){
    var event = "LISTEN";
    msgbus.on( "on", function(data){ deepEqual(event, data);} );
    msgbus.fire( "on", event );
...
test("remove", 1, function(){
    ...
    var listener = msgbus.on  ( "on", function(data){ deepEqual(event, data); } );
    msgbus.fire( "on", event );
    listener.remove();
    msgbus.fire( "on", event );
});
...
test("disable && enable", 2, function(){
    ...
    var event = "DISABLE";
    var msgbus = window.msgbus.getInstance(event);
    var listener = msgbus.on  ( "on", function(data){ num++; } );
    msgbus.fire( "on", event );
    listener.disable();
    msgbus.fire( "on", event );
    equal( num, 1 );
...
test("disable notify", 5, function(){
    ...
    var msgbus = window.msgbus.getInstance(event);
    var listener = msgbus.on( "on", function(data){ num++; }, {enable: "VIEW.SHOW"} );
    equal( num, 0 );
    msgbus.fire( "on", event );
    equal( num, 1 );
...
test("option - once", 2, function(){
    ...
    var event = "ONCE";
    var msgbus = window.msgbus.getInstance(event);
    var listener = msgbus.on( "on", function(data){ num++; }, {once: true} );
    msgbus.fire( "on", event );
    equal( num, 1 );
    msgbus.fire( "on", event );
    equal( num, 1 );
...
test("option - min_interval", 3, function(){
    ...
    var msgbus = window.msgbus.getInstance(event);
    var listener = msgbus.on( "on", function(data){ num++; }, {min_interval: 1000} );
    msgbus.fire( "on", event );
    equal( num, 1 );
    msgbus.fire( "on", event );
    equal( num, 1 );
    setTimeout(function(){ msgbus.fire("on", event); equal(num, 2); start(); }, 1000 );
    stop();
});
...
test("signal", 2, function(){
    var event = "SIGNAL";
    var msgbus = window.msgbus.getInstance(event);
    msgbus.on( "signal", function(data){ deepEqual( event, data ); deepEqual( event, msgbus.signal("signal") ); } );
...
test("error", 1, function(){
    ...
    var ln = msgbus.on("error", function( num ){
